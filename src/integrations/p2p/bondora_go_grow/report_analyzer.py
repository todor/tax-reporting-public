...
def _validate_tax_year(tax_year: int) -> None:
    if tax_year < 2009 or tax_year > 2100:
        raise BondoraGoGrowAnalyzerError(f"invalid tax year: {tax_year}")


def analyze_bondora_go_grow_report(
    *,
    input_pdf: str | Path,
    tax_year: int,
    ...
    try:
        ...
        result = parse_bondora_go_grow_pdf(
            input_pdf=input_pdf,
            secondary_market_mode=secondary_market_mode,
        )
    except BondoraGoGrowAnalyzerError:
        raise
    except (P2PAnalyzerError, P2PValidationError) as exc:
        raise BondoraGoGrowAnalyzerError(str(exc)) from exc
    ...
    output_txt_path = build_appendix6_output_path(
        ...
        output_dir=out_dir,
        stem_fallback="bondora_go_grow_report",
    )
