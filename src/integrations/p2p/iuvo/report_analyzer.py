...
from .iuvo_parser import parse_iuvo_pdf
from .models import IuvoAnalyzerError
...
def _validate_tax_year(tax_year: int) -> None:
    if tax_year < 2009 or tax_year > 2100:
        raise IuvoAnalyzerError(f"invalid tax year: {tax_year}")


def analyze_iuvo_report(
    *,
    input_pdf: str | Path,
    tax_year: int,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="iuvo-report-analyzer")
    parser.add_argument("--input", type=Path, required=True, help="Iuvo profit statement PDF")
...
def main() -> int:
    ...
    try:
        run_result = analyze_iuvo_report(
            input_pdf=args.input,
            tax_year=args.tax_year,
            output_dir=args.output_dir,
            secondary_market_mode=args.secondary_market_mode,
        )
    except IuvoAnalyzerError as exc:
        logger.error("%s", exc)
        print("STATUS: ERROR")
        return 2
