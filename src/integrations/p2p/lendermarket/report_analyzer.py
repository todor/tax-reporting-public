...
from .lendermarket_parser import parse_lendermarket_pdf
from .models import LendermarketAnalyzerError
...
def _validate_tax_year(tax_year: int) -> None:
    if tax_year < 2009 or tax_year > 2100:
        raise LendermarketAnalyzerError(f"invalid tax year: {tax_year}")


def analyze_lendermarket_report(
    *,
    input_pdf: str | Path,
    tax_year: int,
    ...
    try:
        ...
        result = parse_lendermarket_pdf(
            input_pdf=input_pdf,
            secondary_market_mode=secondary_market_mode,
        )
    except LendermarketAnalyzerError:
        raise
    except (P2PAnalyzerError, P2PValidationError) as exc:
        raise LendermarketAnalyzerError(str(exc)) from exc
    ...
    output_txt_path = build_appendix6_output_path(
        ...
        output_dir=out_dir,
        stem_fallback="lendermarket_report",
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="lendermarket-report-analyzer")
    parser.add_argument("--input", type=Path, required=True, help="Lendermarket tax statement PDF")
...
def main() -> int:
    ...
    try:
        run_result = analyze_lendermarket_report(
            input_pdf=args.input,
            tax_year=args.tax_year,
            output_dir=args.output_dir,
            secondary_market_mode=args.secondary_market_mode,
        )
    except LendermarketAnalyzerError as exc:
        logger.error("%s", exc)
        print("STATUS: ERROR")
        return 2
