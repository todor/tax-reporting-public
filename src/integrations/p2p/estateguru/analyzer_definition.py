...
from .report_analyzer import analyze_estateguru_report
...
def _add_arguments(parser: argparse.ArgumentParser, mode: CliMode) -> None:
    add_mode_argument(
        parser,
        mode=mode,
        analyzer_alias="estateguru",
        single_flag="secondary-market-mode",
        type=str,
        help="Estateguru secondary-market mode override",
    )
...
def _build_options(
    ...
    secondary_market_mode = option_value(
        ...
        aggregate_attr="estateguru_secondary_market_mode",
...
ANALYZER = AnalyzerDefinition(
    ...
    description="Estateguru PDF analyzer",
    input_suffixes=(".pdf",),
    detection_token_sets=(("estateguru",),),
