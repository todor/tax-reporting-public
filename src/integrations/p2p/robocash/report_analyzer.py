...
from .models import RobocashAnalyzerError
from .robocash_parser import parse_robocash_pdf
...
def _validate_tax_year(tax_year: int) -> None:
    if tax_year < 2009 or tax_year > 2100:
        raise RobocashAnalyzerError(f"invalid tax year: {tax_year}")


def analyze_robocash_report(
    *,
    input_pdf: str | Path,
    tax_year: int,
    ...
    try:
        ...
        result = parse_robocash_pdf(
            input_pdf=input_pdf,
            secondary_market_mode=secondary_market_mode,
        )
    except RobocashAnalyzerError:
        raise
    except (P2PAnalyzerError, P2PValidationError) as exc:
        raise RobocashAnalyzerError(str(exc)) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="robocash-report-analyzer")
    parser.add_argument("--input", type=Path, required=True, help="Robocash tax report PDF")
