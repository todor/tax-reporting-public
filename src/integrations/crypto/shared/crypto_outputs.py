...
def _row_to_csv_dict(row: IrEnrichedRow) -> dict[str, str]:
    ...
    return {
        ...
        "Quantity": fmt_decimal(ir.quantity, quant=DECIMAL_EIGHT),
        "Proceeds (EUR)": _fmt_opt(ir.proceeds_eur, quant=DECIMAL_EIGHT),
        "Fee (EUR)": _fmt_opt(ir.fee_eur, quant=DECIMAL_EIGHT),
        "Cost Basis (EUR)": _fmt_opt(ir.cost_basis_eur, quant=DECIMAL_EIGHT),
...
def build_ir_run_cli_summary_lines(
    *,
    ...
    lines = [
        ...
        f"manual_check_overrides_rows: {result.summary.manual_check_overrides_rows}",
