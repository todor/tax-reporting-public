...
def run_integration(
    ...
    if integration == "ibkr":
        logger.info(
            "IBKR integration is available via dedicated CLI: "
            "`python -m integrations.ibkr.activity_statement_analyzer` "
    ...
    if integration == "finexify":
        ...
        return 0
    if integration == "afranga":
        logger.info(
            "Afranga integration is available via dedicated CLI: "
            "`python -m integrations.p2p.afranga.report_analyzer` "
            "(input=%s, output=%s, year=%s)",
            input_path,
            output_path,
            year,
